...
from src.friends import match_request_service
from src.friends.schemas import SendMatchRequestSchema, MatchRequestActionResponse

@router.post("/{user_id}/match-request/send", response_model=MatchRequestActionResponse)
async def send_match_request(
    ...
    return await match_request_service.send_match_request(db, user_id, request.target_user_id)


@router.post("/match-request/{request_id}/accept", response_model=MatchRequestActionResponse)
async def accept_match_request(
    ...
    return await match_request_service.accept_match_request(db, request_id, user_id)


@router.post("/match-request/{request_id}/reject", response_model=MessageResponse)
async def reject_match_request(
    ...
    return await match_request_service.reject_match_request(db, request_id, user_id)


@router.delete("/match-request/{request_id}/cancel", response_model=MessageResponse)
async def cancel_match_request(
    ...
    return await match_request_service.cancel_match_request(db, request_id, user_id)


@router.get("/{user_id}/match-requests/pending")
async def get_pending_match_requests(
    user_id: int,
    db: AsyncSession = Depends(get_db)
):
    """Get all pending match requests"""
    return await match_request_service.get_pending_requests(db, user_id)


@router.get("/{user_id}/match-state")
async def get_match_state(
    user_id: int,
    db: AsyncSession = Depends(get_db)
):
    """Get user's current match state"""
    return await match_request_service.get_user_match_state(db, user_id)
